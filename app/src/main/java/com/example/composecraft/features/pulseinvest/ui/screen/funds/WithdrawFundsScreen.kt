...
@Composable
fun WithdrawFundsScreen(vm: FundsViewModel, onBack: () -> Unit) {
    val state by vm.state.collectAsState()
    LaunchedEffect(state.isSuccess) { if (state.isSuccess) { vm.resetSuccess(); onBack() } }

    PulseScaffold(title = "Withdraw Funds", showBack = true, onBack = onBack) { padding ->
        Column(
            modifier = Modifier
            ...
            PulseCard {
                Text("Available to Withdraw", color = PulseTextSecondary, fontSize = 12.sp)
                Spacer(Modifier.height(4.dp))
                Text(
                    "${"$%,.2f".format(state.availableBalance)}",
                    color = PulseGreen, fontWeight = FontWeight.ExtraBold, fontSize = 28.sp
                )
            }
            ...
            listOf(
                Triple("🏦", "Bank Transfer", "ACH transfer to your bank"),
            ...
            PulseButton("Continue", onClick = vm::withdraw, isLoading = state.isLoading)
        }
    }
}
